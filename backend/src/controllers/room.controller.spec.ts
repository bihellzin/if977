...
describe('User Suite', () => {
  beforeAll(async () => {
    ...
    app = new App([new RoomController(), new AuthController(), new GenreController(), new MusicController()]).app;
  ...
  it('Create', async () => {
    ...
    const resMusic = await req.post('/music').send({ name: "Demorô", author: "Criolo", url: "criolo-demoro.mp3", genreId: resAuth.body.data.id })
    .set('Authorization', `Bearer ${token}`);

    expect(resMusic.status).toEqual(201);
    expect(resMusic.body).toBeTruthy();

    const resRoom = await req.post('/room').send({ genreId:resAuth.body.data.id , musicId: resMusic.body.data.id })
    .set('Authorization', `Bearer ${token}`);
  ...
  it('Find One', async() => {
    ...
    const resAuth = await req.post('/genre').send({ name: "Rosque" })
    .set('Authorization', `Bearer ${token}`);
    expect(resAuth.status).toEqual(201);
    expect(resAuth.body).toBeTruthy();

    const resMusic = await req.post('/music').send({ name: "Misery Business", author: "Paramore", url: "paramore-misery-business.mp3", genreId: resAuth.body.data.id })
    .set('Authorization', `Bearer ${token}`);
    ...
    const resFind = await req.get(`/room/${resRoom.body.data.id}`).send()
    .set('Authorization', `Bearer ${token}`);
  ...
  it('Update', async() => {
    ...
    const resRoomUpdate = await req.patch(`/room/${resRoom.body.data.id}`).send({ genreId:resGenre2.body.data.id , musicId: resMusic.body.data.id })
    .set('Authorization', `Bearer ${token}`);
    expect(resRoom.status).toEqual(201);
    expect(resRoom.body).toBeTruthy();
